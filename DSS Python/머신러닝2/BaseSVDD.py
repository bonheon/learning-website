...
def print_all_reg(Y_test,pred):
    ...
    print("model의 G-mean 값은 {:.3f}".format(G_mean))
...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def fit(self, X, y=None, weight=None):
        ...
        self.display = 'off'
        self.predicted_y_ = self.predict(np.asarray(self.X), np.asarray(self.y))
        self.accuracy = np.asarray(accuracy_score(np.asarray(self.y), np.asarray(self.predicted_y_)))
    ...
    def predict(self, X, y=None):
        ...
        if results['exist_y'] == True:
            results['accuracy'] = accuracy_score(np.asarray(results['y']), np.asarray(results['predicted_y']))
    ...
    def _check_X_y(self, X, y):
        ...
        if np.all(tmp_ == np.array([1])) or np.all(tmp_ == np.array([-1])):
            ...
            errorText = "SVDD is only supported for one-class or binary classification. "                        "The label must be 1 for positive samples or -1 for negative samples.\n"
    ...
    def display_fit(self):
        ...
        print('\n')        
        print_all_reg(self.y, self.predicted_y_)
    ...
    def display_predict(self, results):
        ...
        print('\n')  
        print_all_reg(results['y'], results['predicted_y'])
    ...
    def plot_boundary(self, X, y=None, expand_ratio=0.2, n_grids=50, 
        ...
        limit_max = np.max(np.max(X))
        limit_min = np.min(np.min(X))
        ax3.set_xlim(limit_min-4,limit_max+5)
        ax3.set_ylim(limit_min-5,limit_max+5)
        ...
        if self.y_type == 'single':
            ...
            ax3.legend(["정상 (-)", "불량 (+)", "Support vectors"], 
                      ncol=1, loc='upper left', edgecolor='black',
                      markerscale=1.2, fancybox=True) 
...
class BananaDataset():
    ...
    def generate(**kwargs):
        ...
        data_p = data_p + np.random.rand(number, 2) * varBanana*2
        ...
        data_n = data_n + np.random.rand(number, 2)*varBanana*0.5
